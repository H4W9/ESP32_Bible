...
CHARSET = [chr(c) for c in range(0x21, 0x7F)] + list("ÄÖÜäöüß")
...
def main():
    ...
    with open(args.out, "w", encoding="utf-8", newline="\n") as fh:
        ...
        for px in sizes:
            blob, asc, desc, lh = build_vlw(args.ttf, px)
            name = f"vlw_{px}"
        ...
        fh.write("static const VlwFont VLW_FONTS[] = {\n")
        for (name, px, lh, _) in entries:
            fh.write(f"  {{ {name}, {px}, {lh} }},\n")
        fh.write("};\n")
        fh.write(f"static const uint8_t VLW_FONT_COUNT = {len(entries)};\n")
